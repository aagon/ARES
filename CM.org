...
On va pouvoir utiliser la notion de cache : les vidéos sont découpées. CDN : Réseau de distribution, de duplication des donées
...
**** Exemple Adobe Flash Dynamic Streaming

**** Exemple de DASH

DASH n'est pas un protocole, ni un encodeur.

Rien de standardisé sinon les données après mise en forme, et l'annonce de ces données (le fichier manifest).
Les clients sont libres : tout le monde sait coment 


MPD (manifest) est un gros fichier xml, structure :
