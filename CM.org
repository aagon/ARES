...
Il est bien sûr possible d'accéder directement à une machine si on connaît son adresse.

Eventuellement, on voudrait pouvoir donner des noms aux machines.



Aucun mécanisme ne permet de remonter vers un serveur.
...
Ce qu'on fait en LAB. Récupérer des statistiques et même du trafic
...
UDP : minimum vital
TCP : permet la connexion active, et le contrôle de congestion

*** Couche transport

Abstraction de la topologie du réseau : on raisonne de bout en bout.

La couche transport est intégrée dans le système d'exploitation (dans le noyau).

La couche applicative doit être intégrée dans le programme utilisateur.
...
TCP ajoute la fiabilité et ordonnancement, et le contrôle de congestion.

(autres protocoles moins utilisés)
...
- LISTEN (écoute pour établissement de connection)
- SEND
- RECEIVE

*** Multiplexage

Le multiplexage, c'est l'idée qu'on peut regrouper les données demandées par les différents processus d'une même machine.

On doit retrouver le processus distant auquel on veut envoyer le message ou établir la connexion.

Numéro de port permet cette identification.
